const cssProperty = [
    ...
    { num: 2, name: "align-content", desc: " 콘텐츠 사이와 콘텐츠 주위 빈 공간을 플렉스 박스'의 교차축 또는 그리드의 블록 축을 따라 배치하는 방식을 결정합니다." },
    ...
    { num: 94, name: "bottom", desc: " 용기블럭의 높이를 참조합니다. 요소가 밑에서부터 어디쯤 배치 될 지 결정합니다." },
...
const searchList = document.querySelector(".search__list"); //리스트 출력
const searchStart = document.querySelector(".search__box .start") //시작 버튼
...
function startQuiz() {
    ...
    timeInterval = setInterval(reduceTime, 1000); //1초마다 시간이 줄어들게
...
//오답 보여주기
function missAnswer() {
    searchMissAnswer.style.display = "block";

    cssProperty.forEach(item => {
        let answer = item.name.trim().toLocaleLowerCase();
        if (!answers[answer]) {
            searchMissAnswer.innerHTML += `<span>${answer}</span>`
        }
    });
}
